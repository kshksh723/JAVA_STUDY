-- Ïß??õê 1
    ...
    add company_tel varchar2(13);
    
    alter table tbl_company_detail
    modify company_no varchar2(10);
    
    update tbl_company_detail set company_tel = '02-1357-1357'
    where pk_company_code = '05';
    
    commit;
   
    -- Í∏∞ÏóÖ?öå?õêÎ°úÍ∑∏?ù∏ ?Öå?ù¥Î∏? 6
     create table tbl_company_login
    (Pk_Fk_company_code   varchar2(10)     -- Í∏∞ÏóÖÏΩîÎìú
    , company_passwd   varchar2(20)   not null  -- ÎπÑÎ?Î≤àÌò∏
    , company_name      nvarchar2(30)   -- Í∏∞ÏóÖÎ™?
    , Fk_resume_code  varchar2(10)  -- ?ù¥?†•?ÑúÏΩîÎìú
...
where  PK_user_id = 'kimsh' and user_passwd = 'Qwer1234$';
    ...
    values (seq_fk_recruit_notice_code.nextval , '?ã§?†•?ù¥ ?ïàÏ¢ãÏïÑ?èÑ Í¥úÏ∞Æ?ïÑ?öî ?ã†?ûÖ Í∞úÎ∞ú?ûê Î™®Ïã≠?ãà?ã§.','6,000ÎßåÏõê','?õπÍ∞úÎ∞ú Í≤ΩÎ†•?ûê, ?†ïÎ≥¥Ï≤òÎ¶¨Í∏∞?Ç¨ ?ûêÍ≤©Ï¶ù Î≥¥Ïú†?ûê, Í¥??†® ?†ÑÍ≥? Ï°∏ÏóÖ?ûê','Ï£?2?öåÏ∂úÍ∑º, ?ó∞ 1?öå ?õê?ïò?äî ?Ö∏?ä∏Î∂? Ïß?Í∏?, ?Ñ±Í≥ºÍ∏â 3,000%, Î™ÖÏ†à?ÉÅ?ó¨Í∏?, ?Éù?ùº?Ñ†Î¨?, ?ûê?ú†Î°úÏö¥ ?ó∞Ï∞®Î∞òÏ∞?',sysdate , sysdate + 15,'00','03','01','06-01','0','4-1','7','4-2','8','031-03-03','Í≤ΩÎ†• 3?ÖÑ ?ù¥?ïò');