...
public class InputStream_main_01 {
	...
	public static void main(String[] args) throws IOException {
		...
		System.out.println(">> 영문자르 입력하세요 [종료하려면 x를  입력하세요] <<");
		int totalByte = 0;
		int input = 0;
		while(true) {
			...
			if((char) input == 'x' || (char)input == 'X' )
				break;
			System.out.println("글자 1개씩(char)만 읽어들여 char에 해당하는 int 타입으로 반환한 값 : " + input);
			totalByte++;
			
			System.out.println("> 글자 1개 : " + (char)input);
			}
		} // end of while
		System.out.println(">> 입력받은수 : " + totalByte + "byte");
