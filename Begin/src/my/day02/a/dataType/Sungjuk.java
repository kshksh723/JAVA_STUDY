...
public class Sungjuk {
    ...
    -> 암기 */
		...
		// kor + eng + math는 int 타입, int를 강제 형변환하려면 short로 
		...
		int avg1 = total / 3; // 10/4 => 2
						...
						// (double)10/4 => 2.5  10/(float)4 = 2.5  (float)10/(float)4 => 2.5
		//int가 더 큰 범위
		...
		float avg3 = total/3.0F; // 실수 타입, float 4바이트, 소수부 7자리까지만 보여짐 double은 8바이트, 아주 세세하게 표현됌 
		
		double avg4 = (double)total /3; // 정수형 타입인 total을 강제로 double 타입으로 형변환 시키는 것
		// -> total이 사칙연산이 되면서 int가 되기 때문임 (short 여도)
		float avg5 = (float) total / 3;  // 정수형 타입인 total을 강제로 float 타입으로 형변환시키는 것 
		
		long moneny = 300000000L;
		// 자바에서 정수는 기본적으로 int 타입을 취하므로 long 타입으로 나타내려면 
		// 숫자 뒤에 L 또는 l 을 쓰면 된다.
